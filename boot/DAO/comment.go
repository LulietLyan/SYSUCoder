...
func SelectCommentById(id uint64) (entity.Comment, error) {
	...
	tx := database.Db.Where(&entity.Comment{Id: id})
...
func SelectComments(condition model.CommentWhere) ([]entity.Comment, error) {
	...
	tx := database.Db.Model(&entity.Comment{})
	tx = where(tx)
...
func DeleteCommentById(id uint64) error {
	tx := database.Db.Where("id = ?", id).Delete(&entity.Comment{})
	if tx.Error != nil {
		return tx.Error
	}
...
func CountCommentsBetweenCreateTime(startTime time.Time, endTime time.Time) ([]model.CountByDate, error) {
	...
	tx := database.Db.Model(&entity.Comment{}).Where("create_time between ? and ?", startTime, endTime).Select("date(create_time) as date, count(*) as count").Group("date").Scan(&counts)
	if tx.Error != nil {
		return nil, tx.Error
	}
