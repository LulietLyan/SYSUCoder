...
func SelectProblemById(id uint64, condition model.ProblemWhere) (entity.Problem, error) {
	...
	if condition.Testcases.Exist() && condition.Testcases.Value() {
		...
		tx := database.Db.Model(&entity.Testcase{}).Where("problem_id = ?", id).Find(&p.Problem.Testcases)
		if tx.Error != nil {
			return entity.Problem{}, tx.Error
		}
	}
	...
	if condition.Solutions.Exist() && condition.Solutions.Value() {
		...
		tx := database.Db.Model(&entity.Solution{}).Where("problem_id = ?", id).Find(&p.Problem.Solutions)
		if tx.Error != nil {
			return entity.Problem{}, tx.Error
		}
	}
...
func UpdateProblemUpdateTimeById(id uint64) error {
	tx := database.Db.Model(&entity.Problem{}).Where("id = ?", id).Update("update_time", time.Now())
	if tx.Error != nil {
		return tx.Error
	}
...
func DeleteProblemById(id uint64) error {
	tx := database.Db.Where("id = ?", id).Delete(&entity.Problem{})
	if tx.Error != nil {
		return tx.Error
	}
...
func CountProblemsBetweenCreateTime(startTime time.Time, endTime time.Time) ([]model.CountByDate, error) {
	...
	tx := database.Db.Model(&entity.Problem{}).Where("create_time between ? and ?", startTime, endTime).Select("date(create_time) as date, count(*) as count").Group("date").Scan(&countByDate)
	if tx.Error != nil {
		return nil, tx.Error
	}
