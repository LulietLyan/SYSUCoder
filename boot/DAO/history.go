...
func SelectHistoriesByProblemId(pid uint64) ([]entity.History, error) {
	...
	tx := database.Db.Model(&entity.History{}).Where("tbl_history.problem_id = ?", pid)
...
func CountHistoriesBetweenCreateTimeByOperation(startTime time.Time, endTime time.Time, operation entity.Operation) ([]model.CountByDate, error) {
	...
	tx := database.Db.Model(&entity.History{}).Where("create_time between ? and ? AND operation = ?", startTime, endTime, operation).Select("date(create_time) as date, count(*) as count").Group("date").Scan(&countByDate)
	if tx.Error != nil {
		return nil, tx.Error
	}
