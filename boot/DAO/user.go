...
func SelectUserById(id uint64) (entity.User, error) {
	...
	tx := database.Db.Where(&entity.User{Id: id})
...
func SelectUserByEmail(e string) (entity.User, error) {
	...
	tx := database.Db.Where("email = ?", e).First(&user)
	if tx.Error != nil {
		return entity.User{}, tx.Error
	}
...
func SelectUsers(condition model.UserWhere) ([]entity.User, error) {
	...
	tx := database.Db.Model(&entity.User{})
	tx = where(tx)
...
func UpdateUserById(u entity.User) error {
	tx := database.Db.Model(&u).Where("id = ?", u.Id).Updates(u)
	if tx.Error != nil {
		return tx.Error
	}
...
func DeleteUserById(id uint64) error {
	tx := database.Db.Where("id = ?", id).Delete(&entity.User{})
	if tx.Error != nil {
		return tx.Error
	}
...
func CountUsersGroupByRole() ([]model.CountByRole, error) {
	...
	tx := database.Db.Model(&entity.User{}).Select("role, count(*) as count").Group("role").Scan(&counts)
	if tx.Error != nil {
		return nil, tx.Error
	}
...
func CountUsersBetweenCreateTime(startTime time.Time, endTime time.Time) ([]model.CountByDate, error) {
	...
	tx := database.Db.Model(&entity.User{}).Where("create_time between ? and ?", startTime, endTime).Select("date(create_time) as date, count(*) as count").Group("date").Scan(&counts)
	if tx.Error != nil {
		return nil, tx.Error
	}
