...
func SelectSubmissions(condition model.SubmissionWhere) ([]entity.Submission, error) {
	...
	tx := database.Db.Model(&entity.Submission{})
	tx = where(tx)
...
func SelectSubmissionById(id uint64) (entity.Submission, error) {
	...
	tx := database.Db.Where(&entity.Submission{Id: id})
	tx = where(tx)
...
func UpdateSubmissionUpdateTimeById(id uint64) error {
	tx := database.Db.Model(&entity.Submission{}).Where("id = ?", id).Update("update_time", time.Now())
	if tx.Error != nil {
		return tx.Error
	}
...
func DeleteSubmissionById(id uint64) error {
	tx := database.Db.Where("id = ?", id).Delete(&entity.Submission{})
	if tx.Error != nil {
		return tx.Error
	}
